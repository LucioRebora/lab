...
import { Plus, Edit2, Trash2, Microscope } from "lucide-react";
import { MethodModal, type Method } from "@/components/admin/MethodModal";
...
export default function MethodsPage() {
    ...
    const loadMethods = async () => {
        ...
        try {
            ...
            toast.error("Error al cargar los métodos");
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        loadMethods();
        const handleLabChange = () => loadMethods();
    ...
    const handleDelete = async (id: string) => {
        toast("¿Está seguro de eliminar este método?", {
            action: {
                label: "Eliminar",
                onClick: async () => {
                    try {
                        const res = await fetch(`/api/methods/${id}`, { method: "DELETE" });
        ...
        <div className="p-8 max-w-5xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Métodos</h1>
                        <p className="text-sm text-zinc-500 font-medium">Tipos de técnicas analíticas</p>
                    </div>
                </div>
                <button
                    onClick={() => {
                    ...
                    {methods.length > 0 ? (
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-zinc-200 dark:divide-zinc-800">
                                {methods.map((method) => (
                                    <tr key={method.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors group">
