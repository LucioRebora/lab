...
import { X, Save, UserCheck, ShieldAlert, Loader2 } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useSession } from "next-auth/react";
import { toast } from "sonner";
import { Alert } from "@/components/ui/Alert";

export type Biochemist = {
    id: string;
    apellido: string;
    nombre: string;
...
interface BiochemistModalProps {
    open: boolean;
    onClose: () => void;
    onSaved: () => void;
    biochemist?: Biochemist | null;
}

export function BiochemistModal({ open, onClose, onSaved, biochemist }: BiochemistModalProps) {
    ...
    useEffect(() => {
        if (open) {
            if (biochemist) {
                setFormData({
                    apellido: biochemist.apellido || "",
                    nombre: biochemist.nombre || "",
                    tratamiento: biochemist.tratamiento || "Bioq.",
                    codigo: biochemist.codigo || "",
                    direccion: biochemist.direccion || "",
                    ciudad: biochemist.ciudad || "",
                    provincia: biochemist.provincia || "",
                    codigoPostal: biochemist.codigoPostal || "",
                    telefono: biochemist.telefono || "",
                    celular: biochemist.celular || "",
                    email: biochemist.email || "",
                    notas: biochemist.notas || "",
                    firmante: biochemist.firmante || false,
                });
            } else {
                setFormData({
                    apellido: "",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const method = biochemist ? "PATCH" : "POST";
            const url = biochemist ? `/api/biochemists/${biochemist.id}` : "/api/biochemists";
            ...
            toast.success(biochemist ? "Bioquímico actualizado" : "Bioquímico creado");
            onSaved();
            onClose();
        } catch (err: any) {
            ...
            >
                ...
                >
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100">
                                    {biochemist ? "Editar Bioquímico" : "Nuevo Bioquímico"}
                                </h2>
                    ...
                    <div className="p-6 overflow-y-auto max-h-[70vh] custom-scrollbar space-y-8">
                        ...
                        <form id="biochemist-form" onSubmit={handleSubmit} className="space-y-6">
                            {/* Personal Info Group */}
                                ...
                                <div className="grid sm:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <input
                                            ...
                                            className="w-full h-11 px-4 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700/50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all font-medium text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400"
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <div className="flex gap-2">
                                            {["Bioq.", "Dr.", "Dra."].map(trat => (
                                                <button
                                                    key={trat}
                                                    type="button"
                                                    onClick={() => setFormData({ ...formData, tratamiento: trat })}
                                                    className={`px-4 h-11 rounded-2xl text-sm font-medium border transition-colors flex-1 ${formData.tratamiento === trat ? 'bg-orange-50 border-orange-200 text-orange-700 dark:bg-orange-500/10 dark:border-orange-500/20 dark:text-orange-400' : 'bg-zinc-50 border-zinc-200 text-zinc-600 hover:bg-zinc-100 dark:bg-zinc-800/50 dark:border-zinc-700/50 dark:text-zinc-400 hover:dark:bg-zinc-800'}`}
                                                >
                                                    {trat}
                                                </button>
                                            ))}
                                    ...
                                    {/* Código */}
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Código MP/MN</label>
                                        <input
                            ...
                            <div className="space-y-4 pt-4 border-t border-zinc-100 dark:border-zinc-800/50">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full h-24 p-4 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700/50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all font-medium custom-scrollbar resize-none text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400"
                                ...
                                <div className="flex items-center gap-3 pt-2">
                                    <label className="relative flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                                            checked={formData.firmante}
                                            onChange={(e) => setFormData({ ...formData, firmante: e.target.checked })}
                                            className="sr-only peer"
                                            id="firmante-checkbox"
                                        />
                                        ...
                                        <span className="ml-3 text-sm font-medium text-zinc-900 dark:text-zinc-300">
                                            Autorizado como firmante habilitado
                                        </span>
                                    </label>
                                </div>
                            </div>
                    ...
                    <div className="p-6 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 shrink-0 flex justify-end gap-3 rounded-b-4xl">
                        ...
                        >
                            ...
                            {loading ? "Guardando..." : (biochemist ? "Actualizar" : "Guardar Bioquímico")}
                        </button>
                    </div>
                </motion.div>
            </motion.div>
