...
import { X, ShieldAlert, FlaskConical, Loader2, ListOrdered } from "lucide-react";
...
export function UnitModal({ open, onClose, onSaved, unit }: UnitModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const httpMethod = unit ? "PATCH" : "POST";
                ...
                >
                    <div className="flex items-center justify-between p-6 border-b border-zinc-100 dark:border-zinc-800 shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-green-50 dark:bg-green-500/10 flex items-center justify-center">
                                <ListOrdered className="text-green-600 dark:text-green-400" size={20} />
                            </div>
                            <div>
                    ...
                    <div className="p-6 overflow-y-auto max-h-[70vh] custom-scrollbar space-y-8">
                        ...
                        <form id="unit-form" onSubmit={handleSubmit} className="space-y-6">
                            <div className="space-y-1.5">
                                <label className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Nombre de Unidad (Abreviatura) *</label>
                                <input
                                    value={formData.nombre}
                                    onChange={(e) => setFormData({ ...formData, nombre: e.target.value })}
                                    className="w-full h-11 px-4 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700/50 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all font-medium"
                    ...
                    <div className="p-6 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50 shrink-0 flex justify-end gap-3 rounded-b-4xl">
                        ...
                        <button
                            ...
                            form="unit-form"
                            disabled={loading}
                            className="flex items-center gap-2 px-6 py-2.5 bg-green-600 text-white rounded-2xl text-sm font-bold hover:bg-green-700 transition-colors shadow-sm disabled:opacity-50"
                        >
                            {loading && <Loader2 size={16} className="animate-spin" />}
                            {loading ? "Guardando..." : (unit ? "Actualizar" : "Guardar Unidad")}
                        </button>
                    </div>
                </motion.div>
            </motion.div>
