...
async function main() {
    ...
    for (const line of lines) {
        ...
        // Let's create these as global padron items (laboratoryId = null)
        ...
        await prisma.study.upsert({
            ...
            // Prisma might not support null in compound unique depending on DB config in upsert
            // Let's fallback to manual find/update/create if upsert fails
            const ex = await prisma.study.findFirst({ where: { codigo: cod, laboratoryId: null } });
            if (ex) {
